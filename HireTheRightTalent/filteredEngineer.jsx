...
const FilteredEngineer = () => {
  ...
  const { searchedTypeEngineer } = location.state || {
    searchedTypeEngineer: [],
  }; // Get the state
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        ...
        const engineersWithImages = searchedTypeEngineer.map((engineer) => {
          const findById = (collection, id) =>
            collection.find((item) => item._id === id);
          ...
          console.log(`Engineer ID: ${engineer._id}, Image Link: ${imageLink}`);
    ...
    fetchImages();
  }, [searchedTypeEngineer]); // Re-run effect when searchedTypeEngineer changes
...
export default FilteredEngineer;
